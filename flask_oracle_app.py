...
def index():
    if request.method == 'POST':
        ...
        try:
            conn=cx_Oracle.connect('wr/123@localhost:1521')
            cursor=conn.cursor()
            table=[]
            if searchString not in table:
                cursor.execute(f"CREATE TABLE {searchString} (name varchar2(100),rating varchar2(10),commentHead varchar2(255),comtag varchar2(1500))")
                table.append(searchString)
            ...
            for i in range(0,10):
                ...
                for commentbox in commentboxes:
                    ...
                    try:
                        insert_val=f"""INSERT INTO {searchString} VALUES (:1,:2,:3,:4 )"""
                        cursor.execute(insert_val,[name,rating,commentHead,custComment])
                        conn.commit()
